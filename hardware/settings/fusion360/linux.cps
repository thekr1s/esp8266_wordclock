...
properties = {
  ...
  useVacuum: "true" // Enable the vacuum cleaner when spindle starts
};
...
propertyDefinitions = {
  ...
  useVacuum: {title:"Use vacuum cleaner", description:"Enables the vacuum cleaner when spindel starts, and stops it when spindel stops.", type:"boolean"}
};
...
function onSection() {
  ...
  if (properties.useVacuum == "true") {
    writeBlock(mFormat.format(8)) //turn on vacuum
  }
...
function writeRetract() {
  return; //RHU never do a safe retraction, we already retracted when tool is changed
...
function onClose() {
  ...
  if (properties.useVacuum == "true") {
    writeBlock(mFormat.format(9)) //turn off vacuum
  }
